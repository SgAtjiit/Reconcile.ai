...
export async function runLlmMatchPass(
  ...
  for (let i = 0; i < unmatchedSettlements.length; i += chunkSize) {
    ...
    try {
      // Execute OpenRouter LLM call (zero hardcoding)
  ...
  "matches": [
    {
      ...
      for (const m of llmMatches) {
        ...
        if (conf >= threshold) {
          matchedResults.push({
            ...
            explanation: `[OpenRouter LLM: ${model}] ${m.explanation}`,
          });
        } else {
          matchedResults.push({
            batchId,
      ...
      console.warn(`[OpenRouter LLM Fallback] API call deferred/failed (${err.message}). Marking residual chunk as unresolved.`);
      ...
      for (const s of chunkSettlements) {
        ...
        matchedResults.push({
          ...
          confidence: "0.00",
          exceptionCategory: "no_counterpart",
          explanation: `Unresolved: OpenRouter call fallback (${err.message}). Record preserved for manual review.`,
        });
      }
    }
  }
